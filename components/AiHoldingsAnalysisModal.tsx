...
export const AiHoldingsAnalysisModal: React.FC<AiHoldingsAnalysisModalProps> = ({
  ...
  const handleModalShellClose = useCallback(() => {
    ...
    handleClose();
  }, [handleClose, openSessionMenuId, exportMenuOpen, reminderMenuOpen]);
  ...
  return (
    <ModalShell
      isOpen={isOpen}
      onClose={handleModalShellClose}
      overlayId="ai-holdings-analysis-modal"
      zIndex="z-[70]"
      className="relative flex h-[92dvh] w-full flex-col overflow-hidden rounded-t-2xl border border-[var(--app-shell-line)] text-[var(--app-shell-ink)] shadow-[var(--app-shell-shadow)] sm:h-[min(90vh,980px)] sm:w-[1120px] sm:max-w-[1120px] sm:flex-row sm:rounded-2xl"
    >
      <div data-testid="ai-analysis-modal-shell" className="contents">
          ...
          {inspectorOpen && (
            ...
            >
              <header className="flex shrink-0 items-center justify-between border-b border-[var(--app-shell-line)] px-4 py-3">
                <div className="text-sm font-semibold text-[var(--app-shell-ink)]">分析面板</div>
                <button
                  aria-label="收起分析面板"
              ...
              <div className="flex-1 space-y-6 overflow-y-auto p-4">
                <section>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div>
                      ...
                      <div className={`mt-0.5 font-semibold ${getSignColor(summary.holdingGain)}`}>
                        {formatSignedCurrency(summary.holdingGain)}
                      </div>
                    </div>
                    <div>
                      <div className="text-[11px] text-[var(--app-shell-muted)]">日收益</div>
                      <div className={`mt-0.5 font-semibold ${getSignColor(summary.totalDayGain)}`}>
                ...
                <section>
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="mb-1 text-[11px] text-[var(--app-shell-muted)]">资产配置</div>
                      <div ref={allocationChartRef} className="h-48 w-full" />
                    </div>
                    <div>
                      <div className="mb-1 text-[11px] text-[var(--app-shell-muted)]">收益对比</div>
