...
import { analyzeHoldingsChatStream, AiAnalysisMessage, HoldingsSnapshot } from '../services/aiAnalysis';
import { formatCurrency, formatPct, formatSignedCurrency, getSignColor } from '../services/financeUtils';
...
export const AiHoldingsAnalysisModal: React.FC<AiHoldingsAnalysisModalProps> = ({
    ...
    const buildSessionTitle = (index: number) => (
        `${t('common.aiHoldingAnalysisSession') || '会话'} ${index}`
    );

    const createSession = (index: number, title?: string): AiChatSession => {
    ...
    const updateSessionMessages = (sessionId: string, updater: (messages: AiAnalysisMessage[]) => AiAnalysisMessage[]) => {
        if (!sessionId) return;
    ...
    const handleAnalyze = async (forceDefault?: boolean) => {
        ...
        const defaultQuestion = t('common.aiHoldingAnalysisDefaultQuestion') || '请基于当前持仓做一次全面分析（收益、风险、集中度与改进建议）。';
        ...
        const sessionTitle = activeMessages.length === 0
            ? (nextQuestion.length > 12 ? `${nextQuestion.slice(0, 12)}…` : nextQuestion)
            : undefined;
        ...
        const appendAssistant = (text: string) => {
            ...
            if (!finalText) return;
            updateSessionMessages(sessionId, prev => [...prev, { role: 'assistant', content: finalText }]);
            appended = true;
        };
        ...
        try {
            ...
            const reply = answer || streamedText || (t('common.aiHoldingAnalysisEmpty') || '未收到有效回复，请重试');
            ...
            if (!appended && streamedText.trim()) {
                updateSessionMessages(sessionId, prev => [...prev, { role: 'assistant', content: streamedText.trim() }]);
            }
            setIsStreaming(false);
            setStreamingContent('');
                        ...
                        <div className="p-4 overflow-y-auto flex-grow bg-gray-50/50 dark:bg-transparent space-y-4">
                            <div className="bg-white dark:bg-card-dark rounded-xl overflow-hidden shadow-sm p-4 border border-gray-100 dark:border-border-dark">
                                ...
                                <div className="flex flex-wrap gap-3 text-xs mt-2">
                                    <span className={`${getSignColor(summary.holdingGain)} font-sans`}>
                                        {t('common.totalGain') || '持有收益'}: {formatSignedCurrency(summary.holdingGain)} ({formatPct(summary.holdingGainPct)})
                                    </span>
                                    <span className={`${getSignColor(summary.totalDayGain)} font-sans`}>
                                        {t('common.dayGain') || '日收益'}: {formatSignedCurrency(summary.totalDayGain)} ({formatPct(summary.totalDayGainPct)})
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="bg-white dark:bg-card-dark rounded-xl overflow-hidden shadow-sm p-4 space-y-3 border border-gray-100 dark:border-border-dark">
                                ...
                                {hasHoldings && activeMessages.length === 0 && (
                                    <div className="text-xs text-gray-400">
                                        {t('common.aiHoldingAnalysisHint') || '可询问收益结构、风险集中度、持仓分布等。'}
                                    </div>
                                )}
                                        ...
                                        {isStreaming && (
                                            <div className="self-start rounded-lg px-3 py-2 text-sm leading-relaxed bg-gray-50 dark:bg-white/10 text-gray-800 dark:text-gray-100">
                                                {streamingContent ? (
                                                    ...
                                                    <div className="text-xs text-gray-400">{t('common.analyzing') || '分析中...'}</div>
                                                )}
                                            </div>
                                        )}
                                    </div>
