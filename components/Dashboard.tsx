...
export const Dashboard: React.FC = () => {
  ...
  const filterList = ['All', ...accounts.map(a => a.name)];
  ...
  return (
    <div className="pb-36 md:pb-24 bg-app-bg min-h-full" onContextMenu={(e) => e.preventDefault()}>
            ...
            return (
                <button
                    ...
                    className={`flex-shrink-0 px-1 py-2 text-sm font-medium relative whitespace-nowrap transition-colors ${
                        activeFilter === filterKey ? 'text-blue-600' : 'text-gray-500 hover:text-gray-700'
                    }`}
                >
                    {label}
      ...
      <div className="bg-white md:rounded-t-lg px-4 py-3 flex items-center text-xs text-gray-400 border-b border-gray-100 sticky top-[calc(3.5rem+40px)] md:top-14 z-10 shadow-sm font-sans">
         ...
         <div className="hidden md:grid md:flex-[4] w-full grid-cols-6 gap-4 text-right font-medium">
             <div className="text-left">{t('common.cost')} / {t('common.nav')}</div>
             {/* CHANGED: Shares -> Holding Amount */}
             <div className="text-right">{t('common.holdingAmount')}</div>
        ...
        {filteredFunds.map((fund) => {
            ...
            return (
                <div 
                    ...
                    className={`group flex md:flex-row py-4 px-4 border-b border-gray-50 md:border-none active:bg-gray-50 md:hover:bg-gray-50 transition-colors cursor-pointer items-start select-none ${
                        contextMenu?.fundId === fund.id ? 'bg-gray-100' : ''
                    }`}
                >
                    ...
                    {/* Desktop Grid Layout */}
                    <div className="hidden md:grid flex-[4] w-full grid-cols-6 gap-4 text-right items-start text-sm">
                        ...
                        {/* CHANGED: Show Holding Value (Amount) instead of Shares */}
