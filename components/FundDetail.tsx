...
// Check if a date is a holiday using the provided API
const checkIsHoliday = async (dateStr: string): Promise<boolean> => {
    try {
        ...
        console.warn("Holiday API check failed", e);
        return false;
    }
};
...
export const FundDetail: React.FC<FundDetailProps> = ({ fund, onBack }) => {
  ...
  useEffect(() => {
      const fetchCommon = async () => {
          try {
              ...
              // Fallback if common data fails: try to guess or leave empty to trigger legacy fallback
          }
      };
      fetchCommon();
  }, [fund.code]);
  ...
  useEffect(() => {
      const resolveDate = async () => {
          if (lastTradingDay) return; // Already set by Common Data
          ...
          // Fallback: Calculate backwards from YESTERDAY.
          let d = new Date(); 
          ...
          while (attempts < 10) {
              ...
              if (isHoliday) {
                  ...
                  setLastTradingDay(dateStr);
                  break;
              }
          }
      };
      // Only try to resolve if we don't have it yet after a short delay (waiting for common-data)
      if (!lastTradingDay) {
          resolveDate();
      }
  }, [data, lastTradingDay]); 
