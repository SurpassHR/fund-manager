...
export const Header: React.FC<HeaderProps> = ({ title }) => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white border-b border-gray-100 h-14 shadow-sm">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-yellow-100 rounded-full flex items-center justify-center overflow-hidden border border-yellow-300 flex-shrink-0">
             <img src="https://picsum.photos/32/32" alt="Avatar" className="w-full h-full object-cover" />
          </div>
          <div className="hidden sm:flex w-8 h-8 bg-blue-50 rounded-lg flex-col items-center justify-center text-[8px] text-blue-500 leading-tight p-0.5 cursor-pointer hover:bg-blue-100 transition-colors">
             <span className="font-bold">{t('common.list')}</span>
             <span>{t('common.rank')}</span>
          </div>
          <h1 className="text-lg font-bold text-gray-800 ml-1 sm:ml-0">{title}</h1>
        </div>
