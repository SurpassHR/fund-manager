...
export const TransactionHistoryModal: React.FC<TransactionHistoryModalProps> = ({ isOpen, onClose, fund }) => {
                    ...
                    {transactions.length === 0 ? (
                        ...
                        <div className="space-y-3">
                            {transactions.map(tx => (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${tx.type === 'buy'
                                            ? 'bg-red-50 text-red-500 dark:bg-red-900/30'
                                            : 'bg-green-50 text-green-500 dark:bg-green-900/30'
                                            }`}>
                                            <Icons.TrendingUp size={20} className={tx.type === 'sell' ? 'rotate-180 scale-x-[-1]' : ''} />
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className={`text-sm font-bold ${tx.type === 'buy' ? 'text-red-500' : 'text-green-500'}`}>
                                            ...
                                            <div className="text-xs text-gray-400 flex items-center gap-2">
                                                <span>{tx.date} {t(`common.${tx.time}`)}</span>
